...
An abelian group is a set $A$, together with an operation $dot: A times A -> A$ which is associative and commutative. Additionally it has an identity element $a dot e = a$ and for every element $a$ an inverse $a dot a prime = e$. We add a new restriction for our game state to be an abelian group. Similiarly to delta state we define a delta prediction function $f^(Delta p): I^c times S -> Delta S$ producing a delta $Delta p^c_(t,d) = f^(Delta p) (i^c_(t + d), p^c_(t,d))$ instead of the whole state when predicting the next state. Therefore $p^c_(t,d + 1) = p^c_(t,d) + Delta p^c_(t,d)$ and $p^c_(t,d) = s_t + sum^d_(k=0) Delta p^c_(t,k)$. 

The client memorizes $n_"predicted"$ predicted deltas $Delta p^c_(k): t <= k <= t + t^r_c$. We define a new server reconciliation method as follows. For each incoming $Delta s_(t + 1)$ we add a correction update $epsilon.alt = Delta s_(t + 1) - Delta p^c_t$ to our current state $p^c_(t + 1, t^r_c) = p^c_(t, t^r_c) + epsilon.alt$. Assuming that predicition are mostly uncorrelated $Delta p^c_(t,d) approx Delta p^c_(t + 1,d - 1)$ we can show convergence:
$
