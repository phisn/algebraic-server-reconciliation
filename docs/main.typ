...
We previously used an partial application function to merge incoming state from the host with our predicted state. Methods to merge past state with a different predicted state are called server reconciliation. The idea is to reconcile updates from the server with our predicted state.

When merging an update $s^h_(t + 1)$ with our predicted state $p^c_(t + t^r_c)$, we know exactly how we got to our predicted state: by applying all inputs $i^c_k: t <= k <= t + t^r_c$ to the original state $s^p_t$. Instead of overriding the whole predicted state with $s^p_(t + 1)$ only because presumeably wrongly predicted $s^h_(t + 1) != f^p (i^c_(t), s^h_t)$, we can compute what we should have had predicted, given we would have computed the right $s^h_(t + 1) = p^c_(t + 1)$. We can do this by memorizing all unacknowledged inputs, and applying the prediction function once for every input $i^c_k: t + 1 <= k <= t + t^r_c$ on $s^h_(t + 1)$. The result is used to replace our current state. 
